...
public abstract class UniverseTaskBase extends AbstractTaskBase {
    ...
    UniverseUpdater updater =
        new UniverseUpdater() {
          ...
          public void run(Universe universe) {
            ...
            if (err == null) {
              // If error is set, keep reference to the failed task ID.
  ...
  public SubTaskGroup createStartMasterTasks(Collection<NodeDetails> nodes) {
    NodeStatus targetNodeStatus =
        new NodeStatusBuilder().setMasterState(MasterState.Started).build();
    SubTaskGroup subTaskGroup = new SubTaskGroup("AnsibleClusterServerCtl", executor);
    for (NodeDetails node : nodes) {
      AnsibleClusterServerCtl.Params params = new AnsibleClusterServerCtl.Params();
      // Add the node name.
  ...
  public SubTaskGroup createStopServerTasks(
    ...
    NodeStatusBuilder nodeStatusBldr = new NodeStatusBuilder();
    if ("master".equalsIgnoreCase(serverType)) {
      nodeStatusBldr.setMasterState(MasterState.Stopped);
    } else {
      nodeStatusBldr.setTServerState(TserverState.Stopped);
    }
  ...
  public void setNodeState(String nodeName, NodeDetails.NodeState state) {
    ...
    UniverseUpdater updater =
        nodeStateUpdater(
            taskParams().universeUUID,
            nodeName,
            new NodeStatus.NodeStatusBuilder().setNodeState(state).build());
    saveUniverseDetails(updater);
  }
