...
public class TaskExecutor {
   ...
   */
  @FunctionalInterface
  public interface TaskExecutionListener extends TaskCompletionListener {
    void beforeTask(TaskInfo taskInfo);

    default void afterTask(TaskInfo taskInfo, Throwable t) {}
  }
  ...
  public UUID submit(TaskRunner taskRunner) {
    return submit(taskRunner, null);
  }

  // Submit a task to the task executor with a task execution listener.
  public UUID submit(TaskRunner taskRunner, TaskExecutionListener taskExecutionListener) {
    ...
    return submit(taskRunner, executorService, taskExecutionListener);
  }
  ...
  public UUID submit(
    ...
    taskRunner.saveTask();
    TaskContext taskContext = new TaskContext(taskRunner, executorService, taskExecutionListener);
  ...
  public void setSubTaskGroupType(SubTaskGroupType subTaskGroupType) {
    ...
    // TODO Updating the task infos is required to support SubTaskGroup.setSubTaskGroupType.
  ...
  // Submit a subtask task for asynchronous execution from the parent task.
  ...
  public void waitAsyncTasks(TaskCompletionListener listener) {
    waitAsyncTasks(Duration.ZERO, listener);
  }

  // Wait for submitted asynchronous tasks to complete within a timeout
  // with a completion listener callback.
  public void waitAsyncTasks(Duration timeout, TaskCompletionListener listener) {
    Optional<TaskContext> optional = getCurrentTaskContext();
    if (!optional.isPresent()) {
    ...
    try {
      for (SubTaskRunner subTaskRunner : taskContext.subTaskRunners) {
        try {
          ...
          subTaskRunner.setTaskState(TaskInfo.State.Failure);
          subTaskRunner.updateTaskDetailsOnError(e);
          publishAfterTask(taskContext, listener, subTaskRunner.taskInfo, e);
          break;
        }
      }
      // Wait for the existing tasks to finish even if some submissions failed.
   ...
   */
  private void waitForInternal(Duration timeout, TaskCompletionListener listener) {
    Optional<TaskContext> optional = getCurrentTaskContext();
    if (!optional.isPresent()) {
    ...
    while (subTaskRunners.size() > 0) {
      ...
      while (iter.hasNext()) {
        ...
        try {
          ...
          iter.remove();
          publishAfterTask(taskContext, listener, subTaskRunner.taskInfo, null);
        } catch (ExecutionException e) {
          t = e.getCause();
          ...
          if (t != null) {
            ...
            iter.remove();
            anyEx = t;
            publishAfterTask(taskContext, listener, subTaskRunner.taskInfo, t);
          }
        }
      }
    }
  ...
  private void publishBeforeTask(TaskContext taskContext, TaskInfo taskInfo) {
    TaskExecutionListener listener = taskContext.taskExecutionListener;
    if (listener != null) {
      listener.beforeTask(taskInfo);
    }
  }

  private void publishAfterTask(
      TaskContext taskContext, TaskCompletionListener listener, TaskInfo taskInfo, Throwable t) {
    TaskCompletionListener[] listeners =
        new TaskCompletionListener[] {listener, taskContext.taskExecutionListener};
    for (TaskCompletionListener l : listeners) {
      if (l == null) {
        continue;
      }
      l.afterTask(taskInfo, t);
    }
  }
  ...
  public class TaskRunner extends TaskRunnerBase {
    ...
    public void run() {
      ...
      try {
        ...
        publishAfterTask(taskContext, null, taskInfo, null);
      }
    }
